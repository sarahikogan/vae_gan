...
# print outputs to file and terminal
class DualOut:
    def __init__(self, filepath):
        self.terminal = sys.stdout # terminal output
        self.logfile = open(filepath, "w")  

    def write(self, message):
        self.terminal.write(message)  # terminal output
...
def wgan_gp_loss_d(real_scores, fake_scores, gradient_penalty, lambda_gp=10):
    return fake_scores.mean() - real_scores.mean() + lambda_gp * gradient_penalty

def wgan_gp_loss_g(fake_scores): 
...
def train_vae_gan(img_size=128, batch_size=16, epochs=50, latent_dim=128, limit=1000, learning_rate=1e-3, beta=0.1): 
    ...
    for epoch in range(epochs): 
        ...
        for batch in train_loader: 
            ...
            # discriminator loss 
            disc_loss = gan_loss_d(real_labels, fake_labels)
            disc_loss.backward()
            optimizer_disc.step()
            ...
            # generator loss
            gen_loss = gan_loss_g(fake_labels)
            gen_loss.backward()
            optimizer_gen.step()
